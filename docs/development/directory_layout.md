...
| `tests/unit/test_cdp/` | 🟡 进行中 | 已实现机制/组合/analytics/sensitivity UT：`test_mechanisms/test_{laplace,gaussian,exponential,geometric,staircase,vector}.py`、`test_mechanism_factory_registry.py`、`test_composition/test_{basic,advanced,budget_scheduler,privacy_accountant,moment_accountant}.py`、`test_analytics/test_{queries,query_engine}.py`、`test_sensitivity/test_{global_sensitivity,noise_calibrator,sensitivity_bounds,sensitivity_analyzer}.py`、`test_analytics/test_base_generator.py`、`test_analytics/test_synthetic_methods.py`；待补 ML 与高维数据集案例。 |
