...
│       │   ├── 📁 ml/                         # CDP-ML 子系统：DP 训练/推理封装 + 评估/校准（可选模块）
│       │   │   ├── 📄 __init__.py             # 统一导出：DPMLModel 基类、工厂、训练入口、评估入口（避免用户深层导入）
│       │   │   ├── 📄 types.py                # ML 共享类型：TrainBatch/TrainResult/EvalResult/MetricSpec 等结构化数据
│       │   │   ├── 📄 exceptions.py           # ML 统一异常：BackendNotAvailable/InvalidConfig/TrainingError 等
│       │   │   ├── 📄 config.py               # ML 配置：训练超参 + DP 参数 + 会计/预算策略（可序列化/可比较）
│       │   │   ├── 📄 utils.py                # ML 通用工具：seed/device/checkpoint path/json helpers + validation + io
│       │   │   ├── 📁 models/                 # DP 模型抽象与具体模型封装层（对上提供统一 API，对下适配 backends）
│       │   │   │   ├── 📄 __init__.py         # 导出：DPMLModel/LinearModel/.../ModelFactory
│       │   │   │   ├── 📄 base.py             # DPMLModel 抽象基类：fit/predict/score/save/load；声明需要的 backend 能力
│       │   │   │   ├── 📄 linear_model.py     # 线性模型封装：DP Logistic Regression / DP Linear Regression / DP SVM（可选）
│       │   │   │   ├── 📄 neural_network.py   # 神经网络封装：DP-SGD 训练入口（典型对接 torch/tf backend）
│       │   │   │   ├── 📄 tree_model.py       # 树模型封装：DP-GBDT / 私有化分裂策略占位（MVP 可先提供接口+NotImplemented）
│       │   │   │   ├── 📄 clustering.py       # 聚类封装：DP-KMeans 等（可选）；强调“聚类的 DP 输出/中心点噪声”语义
│       │   │   │   └── 📄 model_factory.py    # 模型工厂：根据 config（含 model_type/backend）实例化对应 DPMLModel
│       │   │   ├── 📁 training/               # 训练编排层：训练引擎、DP-SGD 实现、训练指标与隐私事件桥接
│       │   │   │   ├── 📄 __init__.py         # 导出：TrainingEngine / dp_sgd 工具 / TrainResult
│       │   │   │   ├── 📄 training_engine.py  # 训练引擎：统一的 fit 流程编排（数据→backend→callbacks→会计/日志→结果）
│       │   │   │   ├── 📄 dp_sgd.py           # DP-SGD 核心：梯度裁剪 + 噪声添加 + 采样率 + accountant hooks（框架无关部分）
│       │   │   │   ├── 📄 training_metrics.py # 训练期 metrics：loss/acc/auc 等的轻量聚合与滑动统计（不做最终报告）
│       │   │   │   └── 📄 privacy_bridge.py   # 隐私桥：将训练过程中的 DP 消费映射为 core/cdp 的事件（epsilon/delta）
│       │   │   ├── 📁 backends/               # 后端适配层：将 sklearn/torch/tf 的差异收敛为统一接口
│       │   │   │   ├── 📄 __init__.py         # backend 注册与选择：detect_available_backends()/get_backend(name)
│       │   │   │   ├── 📁 sklearn/            # sklearn 后端：偏“传统 ML”封装（LR/LinearRegression/SVM 等）
│       │   │   │   │   ├── 📄 __init__.py     # sklearn backend 入口：能力声明 + wrapper 导出
│       │   │   │   │   ├── 📄 estimator_wrappers.py # sklearn 估计器包装：统一 fit/predict/score；可插入 DP 训练策略
│       │   │   │   │   └── 📄 callbacks.py    # sklearn 训练回调：epoch/iter 级 hooks（若 sklearn 不支持则模拟关键阶段回调）
│       │   │   │   ├── 📁 torch/              # torch 后端：面向 DP-SGD 的主力后端（配合 Opacus/自研 DP-SGD）
│       │   │   │   │   ├── 📄 __init__.py     # torch backend 入口：trainer/模块包装导出 + 能力检查
│       │   │   │   │   ├── 📄 modules.py      # torch 模型组件：简单 MLP/CNN 模板、参数初始化、设备迁移等（可选）
│       │   │   │   │   ├── 📄 trainer.py      # torch 训练执行器：负责实际训练循环（batch/optimizer/scheduler）
│       │   │   │   │   └── 📄 callbacks.py    # torch 回调：on_train_start/step_end/epoch_end + 记录会计/指标
│       │   │   │   └── 📁 tensorflow/         # tensorflow 后端：与 torch 平行（TF Privacy 或自研封装）
│       │   │   │       ├── 📄 __init__.py     # tf backend 入口：trainer 导出 + 能力检查
│       │   │   │       ├── 📄 trainer.py      # tf 训练执行器：tf.data + Keras training loop（或自定义 loop）
│       │   │   │       └── 📄 callbacks.py    # tf 回调：对接 Keras callbacks + DP 会计 hooks
│       │   │   └── 📁 evaluation/             # 评估与校准（不直接生成最终报告）：指标计算、数据切分、DP 参数/预算校准
│       │   │       ├── 📄 __init__.py         # 导出：evaluate()/MetricSpec/Calibrator 等
│       │   │       ├── 📄 metrics.py          # 指标实现：accuracy/f1/auc/mse/mae 等 + 置信区间/重复采样（可选）
│       │   │       ├── 📄 scoring.py          # 评分与汇总：统一把 metrics 汇成 EvalResult（含 per-split/per-class 可选结构）
│       │   │       ├── 📄 data_splitter.py    # 数据切分：train/val/test、k-fold、分层抽样；固定 seed 以保证可复现
│       │   │       ├── 📄 dp_calibrator.py    # DP 校准：给定目标(ε,δ)/效用目标 → 建议 batch_size/noise_multiplier/clip_norm 等
│       │   │       └── 📄 result_exporter.py  # 结果导出：EvalResult/TrainResult → JSON/CSV（轻量）；图表/报告交给 reporting/
│       │   ├── 📁 analytics/                  # CDP分析工具
│       │   │   ├── 📁 queries/                # 隐私保护查询
│       │   │   │   ├── 📄 __init__.py
│       │   │   │   ├── 📄 count.py            # 计数查询
