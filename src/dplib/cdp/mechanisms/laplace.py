...
# 职责：
# - 用 epsilon 与全局敏感度 sensitivity 计算拉普拉斯噪声尺度 scale = sensitivity / epsilon
...
class LaplaceMechanism(BaseMechanism):
    ...
    def _calibrate_parameters(self, *, sensitivity: Optional[float], **kwargs: Any) -> None:
        ...
        self._meta["distribution"] = "laplace"
    ...
    def randomise(self, value: Any) -> Any:
        ...
        result = arr + noise
        return self._restore_numeric_like(value, result, was_scalar)  # 按原始类型恢复（标量/数组等）
    ...
    def deserialize(cls, data: Dict[str, Any]) -> "LaplaceMechanism":
        ...
        # 反序列化：
        # - 使用保存的 epsilon 和 sensitivity 重新构造实例
